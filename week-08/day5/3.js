...
function writeToFileDoubled(readFileName, writeFileName, cb) {
  fs.readFile(readFileName, function (err, content) {
    if (err) {
      return cb(err);
    }
    var doubledText = String(content).repeat(2);
    fs.writeFile(writeFileName, doubledText, function (err, content) {
      if (err) {
        return cb(err);
      }
      cb(null);
...
writeToFileDoubled('alma.txt', 'fruits.txt', function (err) {
  console.log(err);
});
